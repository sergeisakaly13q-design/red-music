...
function ensureSchema() {
  ...
  // Email verification fields/tables for registration.
  try {
    const cols = db.prepare("PRAGMA table_info(users)").all();
    if (!cols.some((c) => c.name === "email")) {
      db.exec("ALTER TABLE users ADD COLUMN email TEXT");
      console.log("[db] Добавлено поле users.email");
    }
    if (!cols.some((c) => c.name === "email_verified")) {
      db.exec("ALTER TABLE users ADD COLUMN email_verified INTEGER NOT NULL DEFAULT 0");
      console.log("[db] Добавлено поле users.email_verified");
    }
    db.exec(`
    ...
    `);
  } catch (e) { console.error("[db] Email verification migration failed:", e.message); }
