...
CREATE TABLE IF NOT EXISTS users (
  ...
  vip_theme      TEXT NOT NULL DEFAULT 'default',
...
-- Stage 2: canonical listening events. Unlike the legacy listening_history table,
-- this table also supports local/uploaded tracks that do not have a server track_id.
...
-- Stage 4: user feedback used by the local recommendation algorithm.
-- One current preference per user/track, so repeated clicks cannot inflate it.
...
-- Verified listening time / achievement progress.
-- A session is counted only after a track reaches its end with sufficient
-- real playback time and without a forward seek jump.
...
CREATE INDEX IF NOT EXISTS idx_listening_reward_sessions_user
  ON listening_reward_sessions(user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_achievement_rewards_telegram
  ON telegram_achievement_rewards(telegram_id, milestone_hour);
...
-- Stage 3: followed artists. One subscription per user/artist.
...
-- Stage 6: friends, activity, now-playing, shared tracks and collaborative playlists.
...
CREATE TABLE IF NOT EXISTS now_playing (
  user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  track_key TEXT NOT NULL, title TEXT DEFAULT '', artist TEXT DEFAULT '',
  playing INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS shared_tracks (
  ...
  track_key TEXT NOT NULL, title TEXT NOT NULL, artist TEXT DEFAULT '',
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
