...
// Render is used only as the Red Music backend. The web interface is bundled
// inside the Android APK and is intentionally never served by this server.
app.get("/", (_req, res) => {
  res.status(403).json({
    error: "APP_ONLY",
    message: "Red Music доступен только через приложение."
  });
});
...
/* Red Music is an app-only backend. The web UI is never served. */
app.use((req, res, next) => {
  const origin = req.headers.origin;
  const allowedOrigins = new Set(["capacitor://localhost","ionic://localhost","https://localhost","http://localhost"]);
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Red-Music-App");
  if (req.method === "OPTIONS") return res.sendStatus(204);
  next();
});

function requireRedMusicApp(req, res, next) {
  if (String(req.headers["x-red-music-app"] || "") !== "android") {
    return res.status(403).json({error:"Red Music доступен только через приложение.",code:"APP_ONLY"});
  }
  next();
}

app.use(["/api","/music","/uploads","/health"], requireRedMusicApp);
...
// Web UI is bundled into the APK and intentionally not exposed by Render.
// Не оставляем Express-страницу "Cannot GET /" для случайных URL.
app.use((req, res) => {
  res.status(404).json({
    error: "NOT_FOUND",
