...
// The mobile/desktop app bundles public/index.html locally through Capacitor/Electron.
// Do not expose the app shell as a website from Render. Static assets remain
// available for API/media use, while the web root and index.html return 404.
app.get(["/", "/index.html"], (_req, res) => {
  res.status(404).json({ ok: false, error: "Red Music app only" });
});
...
app.get("/api/search", (req, res) => {
  ...
  if (!q) return res.json({ ok: true, query: "", tracks: [], artists: [], albums: [], playlists: [] });
  ...
  const score = value => {
    ...
    if (!n) return 0;
    if (n === wanted) return 100;
    if (n.includes(wanted)) return 80;
    let hit = 0;
    for (const t of tokens) if (n.includes(t)) hit++;
    return tokens.length ? Math.round(55 * hit / tokens.length) : 0;
  };
  const tracks = db.prepare(`SELECT id,title,artist,album,filename,mime_type,size_bytes,duration_seconds,created_at FROM tracks ORDER BY created_at DESC LIMIT 500`).all();
  const trackRows = tracks.map(t => ({ ...t, _score: Math.max(score(t.title), score(t.artist)) + (score(t.title) ? 10 : 0) })).filter(t => t._score > 0).sort((a,b)=>b._score-a._score || String(a.title).localeCompare(String(b.title), 'ru')).slice(0, 30);
  const artistMap = new Map();
  for (const t of tracks) { const a=String(t.artist||'').trim(); if(!a) continue; const sc=score(a); if(sc>0 && (!artistMap.has(a) || artistMap.get(a)._score<sc)) artistMap.set(a,{name:a,_score:sc}); }
  const artists=[...artistMap.values()].sort((a,b)=>b._score-a._score || a.name.localeCompare(b.name,'ru')).slice(0,20);
  const albumMap = new Map();
  for (const t of tracks) { const a=String(t.album||'').trim(); if(!a) continue; const sc=score(a); if(sc>0) albumMap.set(a,{name:a,artist:String(t.artist||''),_score:sc}); }
  const albums=[...albumMap.values()].sort((a,b)=>b._score-a._score || a.name.localeCompare(b.name,'ru')).slice(0,20);
  const playlists = db.prepare(`SELECT p.id,p.name,p.source,p.source_url,p.updated_at,COUNT(pt.id) AS track_count FROM playlists p LEFT JOIN playlist_tracks pt ON pt.playlist_id=p.id WHERE lower(p.name) LIKE ? GROUP BY p.id ORDER BY p.updated_at DESC LIMIT 20`).all('%'+wanted+'%');
  res.json({ok:true,query:q,tracks:trackRows.map(({_score,...x})=>x),artists,albums,playlists});
});
...
app.use("/api/recommendations", createRecommendationsRouter(db));
app.use("/api/profile", createProfileRouter(db));
app.use("/api/social", createSocialRouter(db));
...
app.use("/api/vip", createVipRouter(db));
...
// The app shell is intentionally not served by the Render web server.
// Capacitor and Electron load public/index.html from the packaged app.
app.use((req, res, next) => {
  if (req.path.startsWith("/api/")) return next();
  return res.status(404).json({ ok: false, error: "Red Music app only" });
});
