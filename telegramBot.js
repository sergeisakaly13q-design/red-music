...
const ACHIEVEMENT_REFRESH_MS = 15 * 1000;
const ACHIEVEMENT_REFRESH_LIFETIME_MS = 10 * 60 * 1000;

// One temporary refresh timer per Telegram user. It is stopped automatically
// after 10 minutes or when Telegram refuses to edit the message.
const achievementRefreshTimers = new Map();

function clearAchievementRefresh(telegramId) {
  const key = String(telegramId);
  const timer = achievementRefreshTimers.get(key);
  if (timer) clearInterval(timer.interval);
  achievementRefreshTimers.delete(key);
}
...
function getAchievementLinkedAppUser(db, telegramId) {
  return db.prepare(`
    SELECT u.*
...
function getAchievementMinutes(db, appUserId) {
  const stats = db.prepare(`
    SELECT completed_seconds FROM listening_reward_stats WHERE user_id = ?
  `).get(Number(appUserId));
  ...
  // Include the currently playing verified session. The app sends a heartbeat
  // every 10 seconds, so the Telegram display follows playback without waiting
  // for the whole track to finish. Only sessions belonging to this account count.
  const active = db.prepare(`
    SELECT MAX(listened_seconds) AS active_seconds
    FROM listening_reward_sessions
    WHERE user_id = ?
      AND completed = 0
  ...
  const activeSeconds = Math.max(0, Number(active?.active_seconds || 0));
  const totalSeconds = Math.min(
    ACHIEVEMENT_MAX_HOURS * 3600,
    completedSeconds + activeSeconds
  );

  return {
    totalSeconds,
    completedMinutes: Math.floor(totalSeconds / 60),
    completedHours: Math.min(ACHIEVEMENT_MAX_HOURS, Math.floor(totalSeconds / 3600)),
  };
}

function achievementKeyboard(safePage, totalPages) {
  const keyboard = [];
  const buttons = [];
  ...
  keyboard.push([{ text: "🔄 Обновить", callback_data: "ach_refresh" }]);
  keyboard.push([{ text: "👤 Профиль", callback_data: "profile" }]);
  keyboard.push([{ text: "↩️ Главное меню", callback_data: "home" }]);
  return { inline_keyboard: keyboard };
}

async function buildAchievementsMessage(db, telegramId, page = 1) {
  const telegramIdStr = String(telegramId);
  const linked = getAchievementLinkedAppUser(db, telegramIdStr);
  if (!linked) return null;

  await syncListeningAchievements(db, linked.id);

  const progress = getAchievementMinutes(db, linked.id);
  ...
  for (let hour = first; hour <= last; hour += 1) {
    ...
    const current = progress.completedMinutes >= minutes;
    rows.push(
  ...
  const text =
    ...
    `<b>Аккаунт:</b> ${escapeHtml(linked.display_name || linked.username || `ID ${linked.id}`)}\n` +
    `<b>App User ID:</b> <code>${linked.id}</code>\n` +
    `<b>Прослушано:</b> ${progress.completedMinutes.toLocaleString("ru-RU")} мин (${progress.completedHours.toLocaleString("ru-RU")} ч)\n` +
    `<b>Заработано за достижения:</b> ⭐ ${balance.earned_stars || 0}\n` +
    `<b>Максимум:</b> 6 767 ч\n\n` +
    rows.join("\n") +
    `\n\n<b>Страница ${safePage}/${totalPages}</b>\n` +
    `<i>Каждые полные 60 минут прослушивания = ⭐ 10. Прогресс текущего трека обновляется автоматически.</i>`;

  return { text, reply_markup: achievementKeyboard(safePage, totalPages), page: safePage };
}

async function sendAchievements(db, chatId, telegramId, page = 1, options = {}) {
  const telegramIdStr = String(telegramId);
  const linked = getAchievementLinkedAppUser(db, telegramIdStr);

  if (!linked) {
    clearAchievementRefresh(telegramIdStr);
    userStates.set(telegramIdStr, {
      waitingForAchievementAppUserId: true,
      startedAt: Date.now(),
    });
    await sendMessage(chatId,
      `<b>🏆 Достижения Red Music</b>\n\n` +
      `Чтобы открыть достижения, сначала укажите <b>App User ID</b> вашего аккаунта Red Music.\n\n` +
      `Обычный пользователь тоже может пользоваться системой достижений. VIP для этого не требуется.\n\n` +
      `Найдите ID в профиле приложения и отправьте сюда только число.\n` +
      `Пример: <code>123</code>\n\n` +
      `После привязки бот покажет ваши минуты и список наград.`,
  ...
  clearAchievementRefresh(telegramIdStr);
  const view = await buildAchievementsMessage(db, telegramIdStr, page);
  if (!view) return;

  if (options.editMessageId) {
    try {
      await editMessage(chatId, options.editMessageId, view.text, { reply_markup: view.reply_markup });
    } catch (e) {
      // Telegram can reject an edit when the content is unchanged or the message
      // has disappeared. A normal message is safer than leaving the user stuck.
      if (!/message is not modified/i.test(String(e.message || ""))) {
        console.warn("[telegram] Не удалось обновить достижения:", e.message);
      }
    }
  } else {
    ...
    if (sent?.message_id) {
      ...
      const interval = setInterval(async () => {
        const timer = achievementRefreshTimers.get(telegramIdStr);
        if (!timer || Date.now() - startedAt >= ACHIEVEMENT_REFRESH_LIFETIME_MS) {
          clearAchievementRefresh(telegramIdStr);
          return;
        }
        try {
          const current = await buildAchievementsMessage(db, telegramIdStr, view.page);
          if (!current) {
            clearAchievementRefresh(telegramIdStr);
            return;
          }
          await editMessage(chatId, sent.message_id, current.text, { reply_markup: current.reply_markup });
        } catch (e) {
          clearAchievementRefresh(telegramIdStr);
          if (!/message is not modified/i.test(String(e.message || ""))) {
            console.warn("[telegram] Автообновление достижений остановлено:", e.message);
          }
        }
      }, ACHIEVEMENT_REFRESH_MS);
      achievementRefreshTimers.set(telegramIdStr, { interval, messageId: sent.message_id, startedAt, page: view.page });
    }
  }
}

async function handleAchievementAppUserIdInput(db, chatId, telegramId, userInput) {
  const telegramIdStr = String(telegramId);
  const state = userStates.get(telegramIdStr);
  if (!state || !state.waitingForAchievementAppUserId) return false;

  if (Date.now() - Number(state.startedAt || 0) > 30 * 60 * 1000) {
    userStates.delete(telegramIdStr);
    await sendMessage(chatId, `<b>❌ Время ожидания истекло.</b>\n\nОткройте «Достижения» ещё раз.`);
    return true;
  }

  const appUserId = Number.parseInt(String(userInput).trim(), 10);
  if (!Number.isInteger(appUserId) || appUserId <= 0) {
    await sendMessage(chatId,
      `<b>❌ Неверный ID</b>\n\n` +
      `App User ID должен быть положительным числом.\n` +
      `Пример: <code>123</code>`);
    return true;
  }
  ...
  if (!appUser) {
    await sendMessage(chatId,
      ...
      `Проверьте ID в профиле приложения.`);
    return true;
  }
  ...
  if (otherLink) {
    await sendMessage(chatId,
      `<b>❌ Этот аккаунт уже привязан</b>\n\n` +
      `App User ID <code>${appUserId}</code> уже привязан к другому Telegram-аккаунту.`);
    return true;
  }
  ...
  await sendMessage(chatId,
    ...
    `<b>Red Music:</b> ${escapeHtml(appUser.display_name || appUser.username || `ID ${appUserId}`)}\n` +
    `<b>App User ID:</b> <code>${appUserId}</code>\n\n` +
    `Теперь откройте достижения. Минуты будут подтягиваться из приложения автоматически.`,
    { reply_markup: { inline_keyboard: [
      [{ text: "🏆 Открыть достижения", callback_data: "achievements:1" }],
      [{ text: "↩️ Главное меню", callback_data: "home" }],
    ]}});
  return true;
}
...
async function handleMessage(db, message) {
  ...
  // Сначала обрабатываем отдельный ввод ID для достижений.
  const state = userStates.get(String(message.from.id));
  if (state && state.waitingForAchievementAppUserId) {
    await handleAchievementAppUserIdInput(db, message.chat.id, message.from.id, text);
    return;
  }

  // Затем обрабатываем ввод ID после покупки VIP.
  if (state && state.waitingForAppUserId) {
...
async function handleCallback(db, callback) {
  ...
  if (data === "achievements:1") return sendAchievements(db, chatId, from.id, 1);
  if (data === "ach_refresh") {
    const timer = achievementRefreshTimers.get(String(from.id));
    const page = timer?.page || 1;
    return sendAchievements(db, chatId, from.id, page, { editMessageId: chatId === callback.message?.chat?.id ? callback.message.message_id : null });
  }
