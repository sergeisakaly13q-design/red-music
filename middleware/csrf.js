...
function requireCsrfOrigin(req, res, next) {
  ...
  // Native Red Music clients (Capacitor/Electron) can legitimately send
  // state-changing requests without Origin/Referer. The client adds a
  // dedicated header to those requests. A normal cross-site browser request
  // cannot add this header unless the server explicitly allows it through CORS.
