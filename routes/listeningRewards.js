...
module.exports = function createListeningRewardsRouter(db) {
  ...
  router.post("/start", requireAuth, antiSpam, (req, res) => {
    ...
    const artist = String(req.body?.artist || "").slice(0, 200);
    ...
    try {
      ...
      db.prepare(`
        INSERT INTO listening_reward_sessions
          (id, user_id, track_id, client_track_key, client_title, client_artist, duration_seconds,
      ...
      `).run(id, req.userId, trackId, key, title, artist, duration);
  ...
  router.post("/complete", requireAuth, antiSpam, async (req, res) => {
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        // Stage 2 history is written only after the same server-side anti-seek
        // checks that approve the listening reward. This prevents a client from
        // inflating history/statistics by repeatedly seeking or firing play events.
        const duplicate = db.prepare(`
          SELECT id FROM listening_events
          WHERE user_id = ? AND track_key = ?
            AND datetime(played_at) >= datetime('now', '-20 seconds')
          LIMIT 1
        `).get(req.userId, session.client_track_key || `track:${session.track_id || ''}`);
        ...
        if (!duplicate) {
          ...
          `).run(
            ...
            String(session.client_track_key || `track:${session.track_id || ''}`).slice(0, 300),
            String(session.client_title || 'Без названия').slice(0, 200),
            String(session.client_artist || '').slice(0, 200),
            duration,
            Math.min(listened, duration)
          );
        }
      });
      transaction();
