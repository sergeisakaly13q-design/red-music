...
module.exports = function createPlaylistsRouter(db) {
  ...
  router.get("/mine", optionalAuth, (req, res) => {
    ...
    // Отсутствие сессии — это НЕ "у пользователя ноль плейлистов". Раньше здесь
    // возвращался пустой список с кодом 200, и клиент принимал его за истину,
    // затирая локально сохранённые плейлисты после каждого перезапуска сервера.
    if (!owner) return res.status(401).json({ error: "Сессия не найдена", playlistsUnknown: true });
