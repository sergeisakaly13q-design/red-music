...
function parseSpotifyEmbedTracks(html) {
  ...
  for (const j of jsons) collectTrackCandidates(j, candidates, 0);

  // Spotify's embed page has historically exposed track URIs and metadata in
  // JSON blobs even when the normal playlist page is heavily client-rendered.
  // Extract nearby name/artist fields as a fallback without downloading audio.
  const raw = String(html || "");
  const uriRe = /spotify:track:([A-Za-z0-9]{22})/g;
  let m;
  while ((m = uriRe.exec(raw)) && candidates.length < 10000) {
    const id = m[1];
    const windowStart = Math.max(0, m.index - 7000);
    const windowEnd = Math.min(raw.length, m.index + 7000);
    const window = raw.slice(windowStart, windowEnd);
    const names = [];
    const nameRe = /["'](?:name|title)["']\s*:\s*["']((?:\\.|[^"']){1,300})["']/g;
    let nm;
    while ((nm = nameRe.exec(window)) && names.length < 12) names.push(nm[1]);
    const artists = [];
    const artistRe = /["'](?:artistName|subtitle)["']\s*:\s*["']((?:\\.|[^"']){1,220})["']/g;
    let am;
    while ((am = artistRe.exec(window)) && artists.length < 8) artists.push(am[1]);
    const title = names.reverse().find(Boolean) || "";
    const artist = artists.reverse().find(Boolean) || "";
    if (title) candidates.push({ title, artist, externalId: id, sourceUrl: `https://open.spotify.com/track/${id}`, source: "spotify" });
  }
  ...
  for (const item of candidates) {
    ...
    if (!title || (!id && !artist)) continue;
    const key = `${normalizeMatchText(title)}|${normalizeMatchText(artist)}|${id}`;
    if (!dedup.has(key)) dedup.set(key, {
      title,
      artist,
      album: cleanText(item.album || "", 180),
      externalId: id,
      sourceUrl: cleanUrl(item.sourceUrl) || (id ? `https://open.spotify.com/track/${encodeURIComponent(id)}` : ""),
      source: "spotify"
    });
  }
...
async function fetchSpotifyPlaylist(sourceUrl) {
  // Preferred path: official Spotify Web API with free server-side app
  // credentials. No user's Spotify password or OAuth login is required.
  if (process.env.SPOTIFY_CLIENT_ID && process.env.SPOTIFY_CLIENT_SECRET) {
    try {
      return await fetchSpotifyPlaylistViaApi(sourceUrl);
    } catch (apiError) {
      console.warn("[playlist-import] Spotify API failed, using embed fallback:", apiError.message);
    }
  }

  // Credential-free fallback: public Spotify Embed page. This can expose a
  // limited amount of metadata, so the API path above is recommended for
  // large playlists.
  const playlistId = spotifyPlaylistId(sourceUrl);
  ...
  const tracks = parseSpotifyEmbedTracks(html);
  if (!tracks.length) {
    const fallback = parsePage(html, "spotify", sourceUrl);
    if (fallback.tracks.length) return fallback;
    throw new Error("Spotify не отдал список треков. Для полного импорта добавьте SPOTIFY_CLIENT_ID и SPOTIFY_CLIENT_SECRET в переменные окружения сервера.");
  }
  const jsons = extractJsonScripts(html);
  let name = "Импортированный плейлист";
  for (const j of jsons) {
    const candidate = j?.name || j?.title || j?.playlist?.name || j?.playlist?.title;
    if (candidate) { name = cleanText(candidate, 120); break; }
  }
