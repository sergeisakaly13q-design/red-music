...
async function fetchYandexPlaylist(sourceUrl) {
  ...
  try {
    ...
    const tracks = rawTracks.map(yandexTrackFromValue).filter(Boolean).slice(0, 500);
...
module.exports = function createPlaylistsRouter(db) {
  ...
  router.post("/import/save", optionalAuth, (req, res) => {
    ...
    res.json({ ok: true, playlistId, vip, guest: owner.guest, offlineRequested: vip });
  });
  ...
  router.get("/mine", optionalAuth, (req, res) => {
    ...
    const playlists = db.prepare(`SELECT p.*, COUNT(pt.id) AS track_count FROM playlists p LEFT JOIN playlist_tracks pt ON pt.playlist_id=p.id WHERE p.user_id=? GROUP BY p.id ORDER BY p.updated_at DESC`).all(owner.userId || owner.id);
    res.json({ playlists });
  });
