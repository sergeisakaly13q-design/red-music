...
async function fetchSpotifyPlaylist(sourceUrl) {
  ...
  // Do not import a partial or guessed list from the public Spotify page.
  // The embed page is client-rendered and does not provide a reliable
  // complete playlist API. Returning a partial list here is worse than
  // showing an error because it silently creates an incomplete playlist.
  throw new Error("Для полного импорта Spotify-плейлистов добавьте SPOTIFY_CLIENT_ID и SPOTIFY_CLIENT_SECRET в переменные окружения сервера. Без Spotify Web API нельзя надёжно получить все треки плейлиста.");
}
