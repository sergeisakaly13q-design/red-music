...
function cleanText(value, maxLength) {
  ...
  function publicUser(row) {
    return {
      ...
      vipTheme: row.vip_theme || "default",
  ...
  router.post("/register", registerLimit, async (req, res) => {
    try {
      ...
      if (cleanNameValue.length > 10) {
        return res.status(400).json({ error: "Имя пользователя (username) — максимум 10 символов." });
      }
  ...
  router.post("/sync-profile", requireAuth, syncLimit, async (req, res) => {
    try {
      const { name, bio, avatar, avatarColor, vipUntil, vipTheme, avatarShape } = req.body || {};
      ...
      if (nextAvatar.length > 8 * 1024 * 1024) return res.status(400).json({ error: "Аватар слишком большой. Максимум 8 МБ" });
      const vip = roleNames.some((r) => ["VIP","RUBY","CO-CREATOR","OWNER"].includes(r));
      const allowedThemes = new Set(["default","red","pink","blue","purple","green","black","yellow"]);
      const requestedTheme = vipTheme !== undefined ? String(vipTheme || "default") : String(user.vip_theme || "default");
      const requestedShape = avatarShape !== undefined ? String(avatarShape || "circle") : String(user.avatar_shape || "circle");
      const allowedShapes = new Set(["circle","square","flower","clover"]);
      const allowedAvatarColors = new Set([
        "linear-gradient(135deg,#ff0055,#e00000)","linear-gradient(135deg,#7928ca,#ff0080)",
        "linear-gradient(135deg,#0070f3,#00dfd8)","linear-gradient(135deg,#00c853,#00e676)",
        "linear-gradient(135deg,#ff8a00,#ffc107)","linear-gradient(135deg,#ffd600,#ffea00)",
        "linear-gradient(135deg,#00b8d4,#2979ff)","linear-gradient(135deg,#304ffe,#7c4dff)",
        "#ff69b4","#111111","#ffd600"
      ]);
      if (!allowedAvatarColors.has(nextAvatarColor)) return res.status(400).json({ error: "Недопустимый цвет профиля" });
      if (!allowedThemes.has(requestedTheme)) return res.status(400).json({ error: "Недопустимая тема" });
      if (!allowedShapes.has(requestedShape)) return res.status(400).json({ error: "Недопустимая форма аватарки" });
      if (!vip && requestedTheme !== "default") return res.status(403).json({ error: "VIP-темы доступны только VIP и выше" });
      if (!vip && ["flower","clover"].includes(requestedShape)) return res.status(403).json({ error: "Эта форма аватарки доступна только VIP и выше" });
      // vip_until нельзя изменять через клиентскую синхронизацию. Его меняют только
      // серверные механизмы подписки/промокода/админских операций.
      db.prepare(`UPDATE users SET display_name = ?, bio = ?, avatar_url = ?, avatar_color = ?, vip_theme = ?, avatar_shape = ? WHERE id = ?`).run(nextName, nextBio, nextAvatar, nextAvatarColor, requestedTheme, requestedShape, user.id);
