...
module.exports = function createAuthRouter(db) {
  ...
  router.get("/status/:username", (req,res)=>{
    ...
    const user=db.prepare("SELECT id, username, banned, password_hash FROM users WHERE username=?").get(username);
    if(!user)return res.status(404).json({error:"Пользователь не найден"});
    res.json({id:user.id,banned:!!user.banned,passwordRemoved:!!user.password_disabled});
  });
