...
module.exports = function createAuthRouter(db) {
  ...
  router.post("/register", async (req, res) => {
    try {
      ...
      const info = db
        ...
        .run(normalizedUsername, passwordHash, name);
  ...
  router.post("/migrate-login", async (req, res) => {
    try {
      ...
      const token = signToken(user);
      setAuthCookie(res, token);
      res.json({ user: publicUser(user), token, migrated: true });
    } catch (e) {
      console.error(e);
  ...
  router.post("/login", async (req, res) => {
    try {
      ...
      if (!user) {
        logAction(db, null, "login_failed", `Неизвестный логин: ${normalizedUsername}`);
