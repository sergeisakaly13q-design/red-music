...
module.exports = function createSocialRouter(db){
  ...
  router.get('/profile/:userId', requireAuth, (req,res)=>{
    ...
    if(!Number.isInteger(uid)||uid<1) return res.status(400).json({error:'Некорректный пользователь'});
    const u=db.prepare(`SELECT id,username,display_name,bio,avatar_url,avatar_color,profile_public,banned FROM users WHERE id=?`).get(uid);
    if(!u || u.banned) return res.status(404).json({error:'Пользователь не найден'});
    const friendship=db.prepare(`SELECT id,status,requester_id,addressee_id FROM friendships WHERE (requester_id=? AND addressee_id=?) OR (requester_id=? AND addressee_id=?) ORDER BY id DESC LIMIT 1`).get(req.userId,uid,uid,req.userId);
    let status='none'; let friendshipId=null;
    if(uid===req.userId) status='self';
    else if(friendship){friendshipId=friendship.id; status=friendship.status==='accepted'?'accepted':(Number(friendship.requester_id)===Number(req.userId)?'outgoing':'incoming');}
    const publicProfile=Boolean(u.profile_public) || status==='accepted' || uid===req.userId;
    const result={profile:{id:u.id,username:u.username,displayName:u.display_name,bio:u.bio||'',avatarUrl:u.avatar_url||'',avatarColor:u.avatar_color||''},publicProfile,relationship:{status,friendshipId}};
    if(publicProfile){
      const totals=db.prepare(`SELECT COUNT(*) AS tracks,COALESCE(SUM(duration_seconds),0) AS seconds,COUNT(DISTINCT track_key) AS uniqueTracks,COUNT(DISTINCT NULLIF(TRIM(artist),'')) AS uniqueArtists FROM listening_events WHERE user_id=? AND completed=1`).get(uid);
      const likes=db.prepare(`SELECT COUNT(*) AS count FROM favorites WHERE user_id=?`).get(uid);
      const localLikes=db.prepare(`SELECT COUNT(*) AS count FROM recommendation_interactions WHERE user_id=? AND action='like'`).get(uid);
      const days=db.prepare(`SELECT DISTINCT substr(played_at,1,10) AS day FROM listening_events WHERE user_id=? AND completed=1 ORDER BY day DESC LIMIT 366`).all(uid).map(x=>x.day);
      const set=new Set(days); let streak=0; const c=new Date(); c.setHours(0,0,0,0); let day=c.toISOString().slice(0,10); if(!set.has(day)){c.setDate(c.getDate()-1);day=c.toISOString().slice(0,10);} while(set.has(day)&&streak<366){streak++;c.setDate(c.getDate()-1);day=c.toISOString().slice(0,10);}
      const topArtists=db.prepare(`SELECT artist,COUNT(*) AS plays FROM listening_events WHERE user_id=? AND completed=1 AND TRIM(artist)<>'' GROUP BY artist ORDER BY plays DESC,artist COLLATE NOCASE LIMIT 5`).all(uid);
      result.stats={tracks:Number(totals?.tracks||0),minutes:Math.floor(Number(totals?.seconds||0)/60),likes:Math.max(Number(likes?.count||0),Number(localLikes?.count||0)),uniqueTracks:Number(totals?.uniqueTracks||0),uniqueArtists:Number(totals?.uniqueArtists||0),streak};
