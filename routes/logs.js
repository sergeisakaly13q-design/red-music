...
module.exports = function createLogsRouter(db) {
  ...
  router.get("/security", requireAuth, ownerOnly, (req, res) => {
    const limit = Math.min(Math.max(Number(req.query.limit) || 100, 1), 500);
    const events = db.prepare(`
      SELECT se.id, se.ip, se.user_id, se.event_type, se.path, se.method,
             se.status_code, se.details, se.created_at, u.username
    ...
    const blocks = db.prepare(`
      SELECT ip, strikes, blocked_until, last_event_at, reason
  ...
  router.post("/users/:id/delete-password", requireAuth, ownerOnly, (req,res)=>{
    ...
    const info=db.prepare("UPDATE users SET password_disabled=1, password_hash='', session_version=session_version+1 WHERE id=?").run(id);
    if (!info.changes) return res.status(404).json({error:"Пользователь не найден"});
