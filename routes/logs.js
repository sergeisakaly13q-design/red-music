...
module.exports = function createLogsRouter(db) {
  ...
  router.get("/users", (req, res) => {
    ...
    const users = db.prepare(
      `SELECT id, username, display_name, bio, avatar_url, avatar_color, vip_until, banned, created_at
  ...
  router.post("/users/:id/delete-password", (req,res)=>{
    ...
    const info=db.prepare("UPDATE users SET password_disabled=1 WHERE id=?").run(id);
    if (!info.changes) return res.status(404).json({error:"Пользователь не найден"});
    res.json({ok:true});
  });
