...
module.exports = function createMusicRouter(db) {
  ...
  // Public metadata helpers used by the app UI.
  // Artist pictures are proxied by Red Music so Android WebView does not
  // depend on third-party image hotlink/CORS behaviour.
  const artistImageCache = new Map();
  router.get("/artist-image", async (req, res) => {
    const name = String(req.query.name || "").trim().slice(0, 120);
    if (!name) return res.status(400).json({ error: "Не указан исполнитель", image: "" });
    const cacheKey = name.toLowerCase();
    if (artistImageCache.has(cacheKey)) return res.json({ image: artistImageCache.get(cacheKey) || "" });
    try {
      let image = "";
      const deezer = await fetch(`https://api.deezer.com/search/artist?q=${encodeURIComponent(name)}`, {
        headers: { "User-Agent": "RedMusic/1.0" }
      });
      if (deezer.ok) {
        const data = await deezer.json();
        const artists = Array.isArray(data?.data) ? data.data : [];
        const wanted = name.toLowerCase();
        const artist = artists.find(a => String(a?.name || "").toLowerCase() === wanted) || artists[0];
        image = String(artist?.picture_xl || artist?.picture_big || artist?.picture_medium || artist?.picture || "").trim();
      }
      // Fallback: Wikipedia thumbnail when Deezer has no match.
      if (!image) {
        ...
        if (wiki.ok) {
          ...
          image = String(data?.thumbnail?.source || data?.originalimage?.source || "").trim();
        }
      }
  ...
  router.get("/artist-image-file", async (req, res) => {
    const name = String(req.query.name || "").trim().slice(0, 120);
    if (!name) return res.status(400).end();
    try {
      let imageUrl = artistImageCache.get(name.toLowerCase());
      if (!imageUrl) {
        const lookup = await fetch(`https://api.deezer.com/search/artist?q=${encodeURIComponent(name)}`, {
          headers: { "User-Agent": "RedMusic/1.0" }
        });
        if (lookup.ok) {
          ...
          const wanted = name.toLowerCase();
          const artist = artists.find(a => String(a?.name || "").toLowerCase() === wanted) || artists[0];
          imageUrl = String(artist?.picture_xl || artist?.picture_big || artist?.picture_medium || artist?.picture || "").trim();
        }
        if (!imageUrl) {
          const wiki = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(name.replace(/\s+/g, "_"))}`, { headers: { "User-Agent": "RedMusic/1.0" } });
          if (wiki.ok) {
            const data = await wiki.json();
            imageUrl = String(data?.thumbnail?.source || data?.originalimage?.source || "").trim();
          }
        }
        artistImageCache.set(name.toLowerCase(), imageUrl || "");
      }
      if (!imageUrl) return res.status(404).end();
      const imageResponse = await fetch(imageUrl, { headers: { "User-Agent": "RedMusic/1.0" } });
      ...
      console.warn("[Red Music] artist image proxy:", e.message);
      return res.status(404).end();
    }
  });

  // Artist subscriptions used by the Stage 3 artist pages.
  router.get("/artists/follows", requireAuth, (req, res) => {
    const rows = db.prepare("SELECT artist_name AS name, created_at FROM artist_follows WHERE user_id = ? ORDER BY created_at DESC").all(req.userId);
    res.json({ ok: true, artists: rows });
  });

  router.post("/artists/follow", requireAuth, antiSpam, interactionLimit, (req, res) => {
    const name = String(req.body?.artist || "").trim().slice(0, 160);
    if (!name) return res.status(400).json({ error: "Не указан исполнитель" });
    db.prepare("INSERT OR IGNORE INTO artist_follows (user_id, artist_name) VALUES (?, ?)").run(req.userId, name);
    res.json({ ok: true, following: true, artist: name });
  });

  router.delete("/artists/follow", requireAuth, antiSpam, interactionLimit, (req, res) => {
    const name = String(req.body?.artist || "").trim().slice(0, 160);
    if (!name) return res.status(400).json({ error: "Не указан исполнитель" });
    db.prepare("DELETE FROM artist_follows WHERE user_id = ? AND artist_name = ?").run(req.userId, name);
    res.json({ ok: true, following: false, artist: name });
  });

  router.get("/lyrics", async (req, res) => {
    const track = String(req.query.track || "").trim().slice(0, 200);
    const artist = String(req.query.artist || "").trim().slice(0, 160);
    if (!track) return res.status(400).json({ error: "Не указан трек", lyrics: "", syncedLyrics: "" });
    ...
    const artistAliases = (value) => {
      const a = String(value || "").trim();
      const n = normalize(a);
      const out = new Set([a]);
      if (n === "xxxtentacion" || n === "xxx tentacion") {
        out.add("XXXTENTACION");
        out.add("XXXTentacion");
      }
      if (n === "валентин стрыкало") {
        out.add("Валентин Стрыкало");
        out.add("Valentin Strykalo");
      }
      if (n === "madk1d") {
        out.add("madk1d");
        out.add("mad1k");
      }
      if (n === "lil peep") out.add("Lil Peep");
    ...
    const score = (row, wantedTrack, wantedArtist) => {
      const rt = normalize(row?.trackName);
      const ra = normalize(row?.artistName);
      const wt = normalize(wantedTrack);
      const wa = normalize(wantedArtist);
      let points = 0;
      if (rt === wt) points += 100;
      else if (rt.includes(wt) || wt.includes(rt)) points += 55;
      if (ra === wa) points += 80;
      else if (ra.includes(wa) || wa.includes(ra)) points += 35;
      if (row?.syncedLyrics) points += 15;
      if (row?.plainLyrics) points += 5;
      return points;
    };
    ...
    async function fetchJson(url) {
      ...
      try {
        const response = await fetch(url, {
          headers: {
            "User-Agent": "RedMusic/1.1 (lyrics lookup)",
    ...
    try {
      ...
      // Search is more tolerant than /get when album/duration metadata is absent.
      for (const artistVariant of artistAliases(artist)) {
        for (const titleVariant of titleVariants(track)) {
          const params = new URLSearchParams({
            track_name: titleVariant,
            artist_name: artistVariant,
          });
          const rows = await fetchJson(`https://lrclib.net/api/search?${params.toString()}`);
          if (!Array.isArray(rows)) continue;

          for (const row of rows) {
            const candidateScore = score(row, track, artist);
            if (candidateScore > bestScore && (row?.plainLyrics || row?.syncedLyrics)) {
              best = row;
      ...
      // Broad search catches punctuation, feature and metadata differences.
      if (!best || bestScore < 120) {
        const q = encodeURIComponent(`${artist} ${track}`.trim());
        const rows = await fetchJson(`https://lrclib.net/api/search?q=${q}`);
      ...
      let lyrics = String(best?.plainLyrics || "").trim();
      let syncedLyrics = String(best?.syncedLyrics || "").trim();

      // Plain-text fallback. No timestamps are invented, so line highlighting
      // remains available only when genuine synced lyrics exist.
      if (!lyrics && !syncedLyrics) {
        const ovh = await fetchJson(
          `https://api.lyrics.ovh/v1/${encodeURIComponent(artist)}/${encodeURIComponent(track)}`
        );
        lyrics = String(ovh?.lyrics || "").trim();
      }

      return res.json({
        lyrics,
        syncedLyrics,
        source: (lyrics || syncedLyrics) ? (syncedLyrics ? "lrclib" : "lyrics.ovh") : "",
      });
    } catch (e) {
      console.warn("[Red Music] lyrics lookup:", e.message);
      return res.json({ lyrics: "", syncedLyrics: "", source: "" });
    }
  });
