...
module.exports = function createMusicRouter(db) {
  ...
  router.get("/lyrics", async (req, res) => {
    ...
    try {
      const headers = { "User-Agent": "RedMusic/1.0" };
      const params = new URLSearchParams({ track_name: track });
      if (artist) params.set("artist_name", artist);
      let data = null;
      let response = await fetch(`https://lrclib.net/api/get?${params.toString()}`, { headers });
      if (response.ok) data = await response.json();

      // /get can miss tracks because of tiny metadata differences. Search is
      // used as a fallback and still returns syncedLyrics when available.
      if (!data || (!data.plainLyrics && !data.syncedLyrics)) {
        const q = encodeURIComponent(`${artist} ${track}`.trim());
        const search = await fetch(`https://lrclib.net/api/search?q=${q}`, { headers });
        if (search.ok) {
          const results = await search.json();
          const rows = Array.isArray(results) ? results : [];
          const wantedTrack = track.toLowerCase();
          const wantedArtist = artist.toLowerCase();
          data = rows.find(x => String(x?.trackName || "").toLowerCase() === wantedTrack && (!wantedArtist || String(x?.artistName || "").toLowerCase() === wantedArtist))
            || rows.find(x => String(x?.trackName || "").toLowerCase() === wantedTrack)
            || rows[0] || null;
        }
      }

      const lyrics = String(data?.plainLyrics || "").trim();
      const syncedLyrics = String(data?.syncedLyrics || "").trim();
      return res.json({ lyrics, syncedLyrics, source: (lyrics || syncedLyrics) ? "lrclib" : "" });
    } catch (e) {
